"""
Training script for the Vision Transformer steering angle regressor with image preprocessing.
"""
...
def main():
    ...
    # Create data loaders (assuming original images are 480x640)
    train_loader, val_loader, test_loader = create_data_loaders(
        args.data_dir,
        batch_size=args.batch_size,
        val_split=args.val_split,
        test_split=args.test_split,
        img_size=(480, 640),  # Original image size before preprocessing
        seed=args.seed
    )
    ...
    # Create model with target image size after preprocessing
    model = SteeringAngleTransformer(
        image_size=tuple(args.img_size),  # Now 640x480
    ...
    for epoch in range(args.epochs):
        ...
        if val_results['loss'] < best_val_loss:
            ...
            }, os.path.join(args.output_dir, 'best_model_640x480_segmented_cropped.pth'))
    ...
    best_checkpoint = torch.load(os.path.join(args.output_dir, 'best_model_640x480_segmented_cropped.pth'))
